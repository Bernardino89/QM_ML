...
comstructure1 = gcomBuilder.gcomBuilder()
comstructure2 = gcomBuilder.gcomBuilder()
...
def varandas0(initial_guess):
    ...
    comstructure1.readout("starter_1.com")
    comstructure2.readout("starter_2.com")

    for num in range(1):
        comstructure.link0 = '%CHK=dimer_' + str(num) +\
            '.chk \n%MEM=100GB \n%Nprocs=32'
        comstructure.title = str(num) +\
            "_dimer DH-SVPD --> H C N  Def2-SVPD --> Br "
        ...
        comstructure1.link0 = '%CHK=monomer_A' \
            '.chk \n%MEM=100GB \n%Nprocs=32'
        comstructure1.title = str(num) +\
            "_monomer_A  DH-SVPD --> H C N  Def2-SVPD --> Br "

        
        comstructure1.appendix = comstructure1.appendix.replace(
            "ALPHA", '{:1.11f}'.format(abs(initial_guess[0])))
        # print ("appendix :", comstructure.appendix)
        comstructure1.appendix = comstructure1.appendix.replace(
            "BETA", '{:1.11f}'.format(abs(initial_guess[1])))
        ...
        #print ("comstructure.appendix", comstructure1.appendix)
        #comstructure.cleanatoms()  # possibilita di generalizzarlo
        #comstructure.addatoms(tml[0:2])
        #comstructure.appendix = comstructure1.appendix()
        job.prepare(comname="monomer_A.com",
                    logname="monomer_A.log",
                    linejob=comstructure1.combyline()
                    )
        job.launch('g16')
        ...
        comstructure2.link0 = '%CHK=monomer_B' \
            '.chk \n%MEM=100GB \n%Nprocs=32'
        comstructure2.title = str(num) +\
            "_monomer_B  DH-SVPD --> H C N  Def2-SVPD --> Br "
        #tml = comstructure.combyline_conf()
        #comstructure.cleanatoms()  # possibilita di generalizzarlo
        #comstructure.addatoms(tml1[2:5])
        job.prepare(comname="monomer_B.com",
                    logname="monomer_B.log",
                    linejob=comstructure2.combyline()
                    )
        job.launch('g16')
...
def downward(varandas0, gradient_Varandas, initial_guess, epsilon,
    ...
    for I in range(0, MaxIter):
        ...
        while (loop):
            ...
            #if (convergence <= tollerance) or  ( diff_var == 3.772874679341953e-9):
            #    megaloop = False
            #    break
            ...
            if (exponent[0] < 0.1e-1 or exponent[0] > 0.999) or ( exponent[1] < 0.01e-1 or exponent[1] > 0.999):
        ...
        #if (convergence <= tollerance) or  ( diff_var == 3.77e-9):
        #    break
            if (convergence <= tollerance) or  ( diff_var == 1.e-9):
                #megaloop = False
                #loop = True
                megaloop = False
                break
...
if __name__ == "__main__":
    ...
    # start
    initial_guess = np.array(['0.039636841846','0.096543342393'], dtype = np.double )
